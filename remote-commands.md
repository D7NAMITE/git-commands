...
3. (Pushing a new branch) You commit some files to the `dev-foo` branch and try to "push" them to Github, but it fails as shown here:
   ...
   ```
   Explain this error.
   > The error occurs means that the branch `dev-foo` doesn't existed in the remote repository. In this case the
     modification done in the local repository on the `dev-foo` branch won't appear on the remote repository.
